...
from_client :: FromDevice(lb9-eth1, METHOD LINUX, SNIFFER false);
to_server :: Queue -> output2 -> ToDevice(lb9-eth2, METHOD LINUX);


// from server to client
from_server :: FromDevice(lb9-eth2, METHOD LINUX, SNIFFER false);
to_client :: Queue -> output1 -> ToDevice(lb9-eth1, METHOD LINUX);
...
// ARP querier
server_arpq :: ARPQuerier(100.0.0.45, lb9-eth2);
client_arpq :: ARPQuerier(100.0.0.45, lb9-eth1);
...
ip_rewrite :: IPRewriter(round_robin, pattern 100.0.0.45 - - - 1 0);
...
server_classifier[0] -> arp_req2 -> ARPResponder(100.0.0.45 lb9-eth2) -> to_server; //ARP request
...
client_classifier[0] -> arp_req1 -> ARPResponder(100.0.0.45 lb9-eth1) -> to_client; //ARP request
client_classifier[1] -> arp_res1 -> [1]client_arpq; //ARP reply
client_classifier[2] -> ip1 -> Strip(14) -> CheckIPHeader -> IPPrint("IP packet from client") -> client_IP_classifier :: IPClassifier(icmp, dst tcp port 80, -); //IP packet
